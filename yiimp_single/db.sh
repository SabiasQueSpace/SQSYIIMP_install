...
if [[ ! -f "$HOME/sqsyiimp/yiimp_single/.wireguard.install.cnf" ]]; then
  echo "Error: $HOME/sqsyiimp/yiimp_single/.wireguard.install.cnf not found" >&2; exit 1
fi
...
print_info "Restarting MariaDB after configuration update"
...
print_status "Restarting MariaDB service..."
print_info "Restarting MariaDB after phpMyAdmin user creation"
restart_service mariadb
