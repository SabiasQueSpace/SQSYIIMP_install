...
print_msg header "SQSYIIMP post-install checks"

# Environment summary
print_msg status "Install environment"
if [ -n "${VERSION:-}" ]; then
    print_msg info "SQSYIIMP version: ${VERSION}"
else
    record_warning "SQSYIIMP version not found (/etc/yiimpoolversion.conf)"
fi
...
for cmd in yiimpool motd screens stratum addport; do
...
if [ -x "$STORAGE_ROOT/yiimp/site/stratum/runner.sh" ]; then
    record_success "Stratum runner is installed"
else
    record_warning "Stratum runner is missing"
fi

if [ -d "$STORAGE_ROOT/yiimp/site/stratum/services" ]; then
    record_success "Stratum services directory is present"
else
    record_warning "Stratum services directory is missing"
fi
