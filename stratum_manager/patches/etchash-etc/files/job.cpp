...
static bool job_assign_client(YAAMP_JOB *job, YAAMP_CLIENT *client, double maxhash)
{
	...
	bool coin_to_mine;
	if (client->coins_mining_list.empty()) {
		// mc= absent ou vide → le client peut miner tous les coins
		coin_to_mine = true;
	} else {
		coin_to_mine = (std::find(client->coins_mining_list.begin(), client->coins_mining_list.end(), job->coind->symbol) != client->coins_mining_list.end()) ||
	...
	for(CLI li = g_list_coind.first; li; li = li->next)
	{
		...
		for(CLI li = g_list_client.first; li; li = li->next)
		{
			...
			if (client->coins_mining_list.empty()) {
				...
				coin_to_mine = (std::find(client->coins_mining_list.begin(), client->coins_mining_list.end(), coind->symbol) != client->coins_mining_list.end()) ||
...
//					job->id, job->coind->symbol, job->templ->height);
		...
		// is_ethash is also true for Etchash because both use the same
		// Ethereum getWork/job lifecycle. PoW verification remains distinct.
		const bool ethash_job = is_ethash;

		if ((ethash_job && job->templ->height < job->coind->height) ||
		    (!ethash_job && job->templ->height <= job->coind->height)) {
...
//	debuglog("job_update()\n");
	...
	YAAMP_COIND *coind_nojob = NULL;
	bool client_without_job = false;
	bool any_coin_loaded = false;
	...
	if(client_without_job)
	{
		g_list_coind.Enter();

		any_coin_loaded = (g_list_coind.first != NULL);

		for(CLI ci = g_list_coind.first; ci; ci = ci->next)
		{
			YAAMP_COIND *candidate = (YAAMP_COIND *)ci->data;

			if(!candidate)
				continue;

			if(!coind_can_mine(candidate))
				continue;

			coind_nojob = candidate;
			break;
		}
		...
		{
			if(!any_coin_loaded)
				debuglog("clients with no job: no coins loaded\n");
			else
				debuglog("clients with no job: no mineable coin available\n");
		}
		else
		{
			// getblocktemplate / template creation can fail while a coin is syncing.
			if(!coind_create_job(coind_nojob, true))
			{
				debuglog("%s: unable to create job, coin temporarily unavailable\n",
					coind_nojob->symbol);
			}
			else if(!coind_nojob->job)
			{
				debuglog("%s: job creation returned NULL job\n",
					coind_nojob->symbol);
			}
			else if(!job_can_mine(coind_nojob->job))
			{
				debuglog("%s: created job is not mineable\n",
					coind_nojob->symbol);
			}
			else
			{
				// job_assign_clients() manages g_list_client internally,
				// so this must stay outside g_list_client.Enter()/Leave().
				// A valid mineable job exists but at least one client was left
				// unassigned by the normal allocation passes. Only this case
				// represents algorithm overflow.
				g_current_algo->overflow = true;

				job_reset_clients(coind_nojob->job);
				job_assign_clients(coind_nojob->job, -1);
			}
		}
	}
