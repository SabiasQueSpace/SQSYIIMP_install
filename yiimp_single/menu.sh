...
RESULT=$(dialog --stdout --default-item 1 --backtitle "SQSYIIMP • SabiasQue.Space" --title "Network mode • $VERSION" --menu "Choose WireGuard mode for this pool" -1 66 5 \
...
case "$RESULT" in
    1)
        clear;
        echo 'wireguard=true' | sudo -E tee "$HOME"/sqsyiimp/yiimp_single/.wireguard.install.cnf >/dev/null 2>&1;
        echo 'DBInternalIP=10.0.0.2
              server_type=db' | sudo -E tee "$STORAGE_ROOT"/yiimp/.wireguard.conf >/dev/null 2>&1;
        ;;
    2)
        clear;
        echo 'wireguard=false' | sudo -E tee "$HOME"/sqsyiimp/yiimp_single/.wireguard.install.cnf >/dev/null 2>&1;
        ;;
    3)
        clear;
        exit 0;
