...
# Fallback: detect DISTRO from /etc/os-release if preflight.sh did not set it.
# Do not silently map an unknown release to a supported one.
if [ -z "${DISTRO:-}" ]; then
    detect_os_compat
    if [ -z "${DISTRO:-}" ]; then
        print_error "Unsupported operating system: ${OS_PRETTY_NAME:-unknown}"
        exit 1
    fi
fi
