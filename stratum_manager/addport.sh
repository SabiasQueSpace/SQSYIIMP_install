...
# Addport historically generated one control script per coin:
#
#   /usr/bin/stratum.COIN
#
# New wrappers are normalized automatically when Addport exits.
# Only configurations containing an explicit [RUNTIME] binary
# are migrated, so old/custom configurations are never guessed.
#
# Supported commands:
#
#   stratum.COIN start
...
sqsyiimp_refresh_stratum_wrappers() {
    ...
    for control in "$config_dir"/stratum.*; do
        ...
        # Session names are intentionally restricted.
        [[ "$coin" =~ ^[A-Za-z0-9_-]+$ ]] || continue

        config=""

        # Configuration generated by the new wrapper format.
        config="$(
            sed -nE \
        ...
        # Configuration generated by the legacy Addport wrapper.
        if [[ -z "$config" ]]; then
            config="$(
                sed -nE \
        ...
        # Last-resort lookup. Only accept exactly one candidate.
        if [[ -z "$config" ]]; then
        ...
        # Never guess which executable a legacy configuration needs.
        [[ -n "$runtime" ]] || continue
...
stratum_stop() {
    ...
    if stratum_screen_running; then
        echo "ERROR: ${SESSION^^} screen is still running."
        return 1
    fi
...
stratum_status() {
    ...
    if pgrep -af -- "$CONFIG" >/dev/null 2>&1; then
        echo "Process : RUNNING"
        pgrep -af -- "$CONFIG" || true
    else
...
# Normalize generated wrappers whenever Addport finishes.
trap 'sqsyiimp_refresh_stratum_wrappers' EXIT
