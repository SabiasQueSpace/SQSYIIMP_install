...
print_header "SQSYIIMP single-server installation"
...
print_message() {
    clear
    echo -e "${YIIMP_HEADER}"
    echo -e "${YIIMP_BLUE}║${YIIMP_RESET}$(center_text "${YIIMP_GREEN}SQSYIIMP ${VERSION} — SabiasQue.Space${YIIMP_RESET}")"
    echo -e "${YIIMP_BLUE}║${YIIMP_RESET}$(center_text "${YIIMP_CYAN}Thank you for using SQSYIIMP!${YIIMP_RESET}")"
