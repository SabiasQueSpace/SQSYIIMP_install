...
#
# This is the main Options menu for SQSYIIMP.
...
show_menu() {
    RESULT=$(dialog --stdout --backtitle "SQSYIIMP • SabiasQue.Space" --title "Manage & Upgrade • $VERSION" --menu "Upgrades and maintenance tools" -1 68 10 \
        ' ' "═══════════  Upgrade ═══════════" \
        1 "Upgrade SQSYIIMP Installer" \
    ...
    case "$RESULT" in
        ...
        4)
            ...
            cd "$HOME/sqsyiimp/yiimp_upgrade/utils"
            source restore.sh
            exit 0
            ;;
        5)
